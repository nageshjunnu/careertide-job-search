...
export function AutomationDashboard() {
  ...
  return <>
    ...
    <main className="automation-shell">
      ...
      {runProgress && <section className={`live-run-progress ${runProgress.running ? 'running' : 'complete'}`} role="status"><div><span>{runProgress.running ? 'CAREER INTELLIGENCE RUN' : 'CAREER INTELLIGENCE COMPLETE'}</span><strong>{runProgress.stage}</strong><small>{runProgress.running ? 'Rules, match threshold, duplicate checks, and permitted sources are being processed.' : 'Your career intelligence pipeline and database status are up to date.'}</small></div><b>{runProgress.percent}%</b><i><em style={{ width: `${runProgress.percent}%` }} /></i></section>}
      ...
      <section className="automation-grid">
        ...
        <aside className="automation-side">
          <article className="automation-panel deposit-card"><div className="deposit-title"><span>₹</span><div><small>CareerTide membership</small><h2>₹1,000 / month</h2></div></div><p>Your monthly plan enables guided discovery, matching, and workflow tools. Payments are verified through Razorpay.</p><ul><li>✓ Activity transparently tracked</li><li>✓ Eligibility visible in dashboard</li><li>✓ Secure subscription verification</li></ul>{depositVerified ? <div className="verified-payment">✓ Monthly plan verified</div> : <Button onClick={verifyDeposit}>Review membership</Button>}</article>
