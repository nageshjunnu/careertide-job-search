...
export const setupApi = {
  ...
  billingStatus: (userId: string) => makeApiCall<{ billing: { status: string; period_end: string | null; advance_months: number; included_jobs: number; used_jobs: number }; pricing: { monthlyAmount: number; includedJobs: number; extraJobAmount: number; firstConnectionAmount: number; accountChangeAmount: number } }>(`${API_URL}/api/payments/billing/${userId}`),
  cancelBilling: (userId: string) => makeApiCall<{ cancelled: boolean; message: string }>(`${API_URL}/api/payments/billing/${userId}/cancel`, { method: 'POST' }),
  createPaymentOrder: (amount = 100000) => makeApiCall<{ orderId?: string | null; subscriptionId?: string | null; checkoutKey: string; amount: number; currency: string; keyId: string; mode: string }>(`${API_URL}/api/payments/order`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount }), timeoutMs: 30_000 }),
  verifyRazorpayPayment: (payment: { razorpay_order_id?: string; razorpay_subscription_id?: string; razorpay_payment_id: string; razorpay_signature: string; userId?: string; amount?: number }) => makeApiCall<{ verified: boolean; paymentId: string }>(`${API_URL}/api/payments/verify`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payment), timeoutMs: 30_000 }),
  ...
  updateSourceStatus: (userId: string, source: string, enabled: boolean) => makeApiCall<{ updated: boolean }>(`${API_URL}/api/career/sources/${userId}/${encodeURIComponent(source)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ enabled }) }),
  requestAccountChangeOtp: (userId: string, source: string) => makeApiCall<{ sent: boolean; message: string }>(`${API_URL}/api/career/sources/${userId}/${encodeURIComponent(source)}/change-account/otp`, { method: 'POST' }),
  verifyAccountChangeOtp: (userId: string, source: string, code: string) => makeApiCall<{ verified: boolean }>(`${API_URL}/api/career/sources/${userId}/${encodeURIComponent(source)}/change-account/verify`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code }) }),
