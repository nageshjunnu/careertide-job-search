...
export const setupApi = {
  ...
  paymentStatus: () => makeApiCall<{ configured: boolean; mode: string; keyId: string | null }>(`${API_URL}/api/payments/status`),
  createPaymentOrder: () => makeApiCall<{ orderId: string; amount: number; currency: string; keyId: string; mode: string }>(`${API_URL}/api/payments/order`, { method: 'POST', timeoutMs: 30_000 }),
  verifyRazorpayPayment: (payment: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => makeApiCall<{ verified: boolean; paymentId: string }>(`${API_URL}/api/payments/verify`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payment), timeoutMs: 30_000 }),
