...
export const adminApi = {
  ...
  candidateAnalyticsDetail: (token: string, userId: string) => makeApiCall<{ payments: Array<{ payment_id: string; amount: number; mode: string; status: string; verified_at: string | null; created_at: string; months_covered: number }>; daily: Array<{ day: string; fetched: number; matched: number; applied: number }> }>(`${API_URL}/api/admin/candidates/${userId}/analytics-detail`, { headers: authHeaders(token) }),
