    ...
    <main className="admin-shell">
      <aside className={menuOpen ? 'open' : ''}>
        <div className="admin-brand">
          {siteLogo ? <img src={siteLogo} alt="" style={{ width: 26, height: 26, objectFit: 'contain' }} /> : <ShieldCheck />}
...
function AdminContent({
  ...
  if (section === 'analytics') {
    ...
    if (candidateId) return <section className="admin-content"><button className="workflow-control" onClick={() => window.history.back()}>← Back to analytics</button>{detail ? <><Panel title={`${detail.full_name} — candidate dashboard`} subtitle={`${detail.email} · complete individual analytics and controls`}><div className="admin-user-actions" style={{ padding: '14px 18px' }}><button className="workflow-control" onClick={() => candidate && onManage(candidate)}><SlidersHorizontal /> Edit candidate settings</button></div><CandidateAnalyticsTable analytics={[detail]} onView={() => {}} /><div className="admin-settings-grid"><div><strong>Profile & contact</strong><span>Email: {detail.email}<br />Phone: {candidate?.phone || '—'}<br />Joined: {candidate ? new Date(candidate.created_at).toLocaleString() : '—'}</span></div><div><strong>Job preferences</strong><span>Roles: {candidate?.roles || '—'}<br />Experience: {candidate?.experience || '—'}<br />Locations: {candidate?.locations || '—'}</span></div><div><strong>Automation controls</strong><span>Workflow: {candidate?.workflow_status || '—'}<br />Schedule: {candidate?.schedule || '—'}<br />Daily limit: {candidate?.daily_limit ?? '—'}<br />Minimum score: {candidate?.minimum_score ?? '—'}</span></div></div></Panel></> : <AdminSkeleton />}</section>
